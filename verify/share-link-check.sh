...
clash_xhttp_tls="$(xray_agent_build_clash_meta_vless "vless_xhttp" "11111111-1111-1111-1111-111111111111" "tls")"
sing_box_xhttp_reality="$(xray_agent_build_sing_box_vless "vless_xhttp" "11111111-1111-1111-1111-111111111111" "reality")"

[[ "${uri_tcp}" == *":9443?"*type=tcp* ]]
[[ "${uri_reality}" == *security=reality* ]]
[[ "${uri_xhttp_tls}" == *"@example.com:443?"*type=xhttp* ]]
[[ "${uri_xhttp_reality}" == *"@203.0.113.10:443?"*pbk=public-key* ]]

jq -e '.port == 443 and .network == "xhttp"' <<<"${clash_xhttp_tls}" >/dev/null
jq -e '.server_port == 443 and .transport.type == "xhttp"' <<<"${sing_box_xhttp_reality}" >/dev/null
