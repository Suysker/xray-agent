...
如果已在网站/反代管理中注册本机真实网站，脚本会优先把该站点 Host 作为 Reality 默认建议，但不会静默覆盖已有 Reality 配置。Reality 目标仍需确认其证书链、TLS 行为和 serverNames 适合当前线路。
...
Hysteria2 的默认伪装 URL 会优先使用已注册真实网站；没有真实网站时再使用历史配置、Nginx fallback、Reality target 或外部兜底。证书域名仍使用安装域名，伪装 URL 可在安装/重配时手动覆盖。

## Nginx 前门 PROXY protocol

Nginx stream 前门的 PROXY protocol 是全局开关，不能对不同 HTTPS 后端细粒度混用。脚本默认使用 `auto`：能安全开启时开启，检测到未知或不支持的 HTTPS 后端时关闭。

菜单切换该模式时，会同时更新 `alone.stream` 和 Xray TLS/Reality inbound 的 `acceptProxyProtocol`。宝塔、1Panel、OpenResty、Caddy、Apache 等第三方配置只做检测和接入提示，脚本不会自动改写它们的配置文件。
