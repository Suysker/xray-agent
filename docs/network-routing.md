...
菜单 `8.WARP 分流及中国大陆域名/IP` 会显示当前 WARP 接口能力：
...
菜单 `7.黑名单和中国大陆 IP 策略` 可用于管理黑名单和中国大陆 IP 策略。菜单会展示当前规则数量和出站状态，避免重复添加规则。

## 本机转发地址

脚本会根据系统能力选择本机服务之间的转发地址：

- IPv4 可用时优先使用 `127.0.0.1`。
- IPv6-only 或无 IPv4 回环时使用 `::1`。

这类选择由脚本自动完成，用户通常只需要确认域名解析和防火墙规则。
